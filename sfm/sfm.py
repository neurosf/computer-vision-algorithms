...
K = np.array([[2393.952166119461, -3.410605131648481e-13, 932.3821770809047],\
               [0, 2398.118540286656, 628.2649953288065],
                 [0, 0, 1]])"""
K = np.array([[9.51226811e+02, 0.00000000e+00, 2.79936274e+02],
       [0.00000000e+00, 1.16325117e+03, 7.16880589e+02],
...
img_dir = 'images/Object1'

# A provision for bundle adjustment is added, for the newly added points 
# from PnP, before being saved into point cloud. 
# Note that it is still extremely slow
# To reduce drift, and to obtain a more robust pipeline we need to periodically
# make a global refinement of all camera poses and all 3D points
# Bundle adjustment implies that we have two sets:
# • P is a set of reconstructed 3D points
# • Q is a set of images or camera views
# • For each view in Q there exists an estimate of the corresponding camera pose, 
#   and visible image points
# • All 3D positions in P and poses in Q are expressed in a global coordinate 
# system, e.g., one of the camera coordinate systems
# The matching between P and Q can be measured in terms of the total squared
# re‐projection error ε
# bundle adjustment minimizes the re-projection error by adjusting both
...
# A function, for triangulation, given the image pair and their corresponding 
# projection matrices
...
def to_ply(path, point_cloud, colors, densify):
    ...
    ply_header = '''ply
		...
		'''
    if not densify:
        with open(path + '/Point_Cloud/sparseObj1.ply', 'w') as f:
...
# function returns the rotation from camera1 to camera2 if points1 are 
# found on camera1 and points2 are found on camera2.
# The returned translation vector is also from camera1 to camera2 but 
# in the coordinate frame of camera1.
# The translation vector is found only up to a factor which can't be 
# recovered without further logic.
